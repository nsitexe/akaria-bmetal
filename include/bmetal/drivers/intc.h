...
struct k_intc_driver_ops {
	int (*add_handler)(struct k_intc_device *intc, int event, struct k_event_handler *handler);
	int (*remove_handler)(struct k_intc_device *intc, int event, struct k_event_handler *handler);
...
static inline int k_intc_add_handler(struct k_intc_device *intc, int event, struct k_event_handler *handler)
{
	return -ENOTSUP;
}

static inline int k_intc_remove_handler(struct k_intc_device *intc, int event, struct k_event_handler *handler)
{
	return -ENOTSUP;
}

static inline int k_intc_handle_generic_event(struct k_intc_device *intc, int event, struct k_event_handler *hnd_head)
{
	return -ENOTSUP;
}
